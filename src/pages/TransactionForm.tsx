...
export default function TransactionForm() {
  ...
  return (
    <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col px-5 pt-3 pb-24 bg-[#151518] space-y-4">
      ...
      <div className="bg-[#1F2024] border border-neutral-800 rounded-[24px] p-1 flex">
        {(['expense', 'income'] as const).map((t) => (
          <button
            ...
            onClick={() => setType(t)}
            className={cn(
              'flex-1 py-3 rounded-[20px] text-sm font-medium transition-all',
      ...
      {/* Amount */}
      <div className="bg-[#1F2024] border border-neutral-800 rounded-[24px] p-5 space-y-1">
        <label className="text-xs text-neutral-500 uppercase tracking-wider">Amount</label>
        <div className="flex items-baseline space-x-1">
          <span className="text-2xl text-neutral-500 font-medium">$</span>
          <input
            type="number"
            placeholder="0.00"
            value={amount}
            onChange={(e) => setAmount(e.target.value)}
            className="flex-1 bg-transparent text-3xl font-bold text-white placeholder:text-neutral-700 focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="bg-[#1F2024] border border-neutral-800 rounded-[24px] p-5 space-y-1">
        ...
        <input
          ...
          className="w-full bg-transparent text-[15px] text-white placeholder:text-neutral-600 focus:outline-none"
        />
      </div>

      {/* Category */}
      <div className="bg-[#1F2024] border border-neutral-800 rounded-[24px] p-5 space-y-2">
      ...
      <button
        ...
        className={cn(
          'w-full py-4 rounded-[20px] text-[15px] font-semibold transition-all active:scale-[0.98]',
